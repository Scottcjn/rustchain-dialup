#!/usr/bin/env python3
"""
RustChain BBS Door — wallet balance + attestation status + mine-while-you-read panel.
...
# ---------------------------------------------------------------------------
def _get_json(url: str, timeout: float = 10.0, tls_verify: bool = False) -> tuple[int, dict]:
    """GET a JSON endpoint. Returns (status_code, parsed_json)."""
...
def render_mining_panel(client: RustChainClient, width: int = 60) -> str:
    ...
    lines.append(_box_line(
        f"  {BRIGHT_WHITE}{BOLD}{STAR} MINE WHILE YOU READ{RESET}", width, BRIGHT_GREEN
    ))
    ...
    if has_error:
        ...
        # Epoch progress
        epoch_pct = (slot / total_slots * 100) if total_slots > 0 else 0
        bar = _progress_bar(slot, total_slots, width=25)

        lines.append(_box_line(
            f"  {WHITE}Epoch:     {BRIGHT_YELLOW}{BOLD}#{epoch_num}{RESET}", width, GREEN
        ))
        lines.append(_box_line(
            f"  {WHITE}Slot:      {slot}/{total_slots} ({epoch_pct:.1f}%){RESET}", width, GREEN
        ))
        lines.append(_box_line(
